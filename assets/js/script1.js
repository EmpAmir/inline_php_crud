  ...
  $("#form-data").submit(function (e) {
    ...
    check_fields.then(function (resp) {
      ...
      if (!IsEmail(data["email"])) {
        ...
        $('[name="email"][contenteditable]').removeClass(
      ...
      if (!isContact(data["contact"])) {
        ...
        $('[name="contact"][contenteditable]').removeClass(
          "bg-danger text-light bg-opacity-50"
        );
      }
      $.ajax({
        url: "./api1.php?action=save",
        method: "POST",
        data: data,
        dataType: "json",
        error: (err) => {
...
window.isContact = function (contact) {
  return $.isNumeric(contact) && contact.length == 10;
};
